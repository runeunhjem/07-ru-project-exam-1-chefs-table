...
const apiUrl = 'https://example.com/wp-json/wp/v2/posts?per_page=4'; // API URL to fetch the first 4 posts
...
async function getPosts() {
  try {
    ...
    // Process the first 4 posts and add them to the `posts` array
    for (const item of result) {
      const categoriesResponse = await fetch(item._links["wp:term"][0].href);
      const categoriesResult = await categoriesResponse.json();
      const categories = categoriesResult.map(category => category.name);
    ...
    // Fetch the remaining posts in the background and add them to the `posts` array
    const remainingResponse = await fetch('https://example.com/wp-json/wp/v2/posts?offset=4');
    const remainingResult = await remainingResponse.json();
    console.log(remainingResult);

    for (const item of remainingResult) {
      const categoriesResponse = await fetch(item._links["wp:term"][0].href);
      const categoriesResult = await categoriesResponse.json();
      const categories = categoriesResult.map(category => category.name);
