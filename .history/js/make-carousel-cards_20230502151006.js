  ...
  for (const post of posts) {
    ...
    postContainer.addEventListener("touchmove", (event) => {
      if (isSwiping) {
        ...
        postContainer.style.transform = `translateX(-${currentPosition * cardWidth * 3 + diffX}px)`; // move postContainer
      }
    });
    ...
    postContainer.addEventListener("touchend", () => {
      if (isSwiping) {
        ...
        if (diffX > 0) {
          const maxPosition = Math.ceil((posts.length - 1) / 4);
        ...
        postContainer.style.transform = `translateX(-${currentPosition * cardWidth}px)`; // move postContainer to final position
