...
commentForm.addEventListener("submit", async function (event) {
  ...
  if (t.length > 0 && o.length > 0)
    try {
      E.disabled = !0;
      const fetchCommentApi = await new $("https://wordpress.runeunhjem.no/wp-json/wp/v2/").post(`comments?post=${postId}`, {
        post: postId,
        author_name: `${commentName.value}`,
        author_email: `${commentEmail.value}`,
        content: `${commentContent.value}`,
      });
      console.log(fetchCommentApi);
    } catch (error) {
      console.log(error);
    } finally {
      console.log();"Success! comment pending approval");
    }
});
