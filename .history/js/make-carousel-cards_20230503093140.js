...
const mediaQueries = Object.keys(breakpoints).map((key) => {
  return window.matchMedia(`(max-width: ${breakpoints[key]}px)`);
});

mediaQueries.forEach((mediaQuery, index) => {
  console.log(mediaQueary is);
...
const smallestMediaQuery = window.matchMedia(`(max-width: ${breakpoints.small - 1}px)`);
