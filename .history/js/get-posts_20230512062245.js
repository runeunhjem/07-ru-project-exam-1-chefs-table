...
async function getPosts() {
  let postsPerPage = 50;
  try {
    const apiUrl = `https://wordpress.runeunhjem.no/wp-json/wp/v2/posts?per_page=${postsPerPage}`;
    ...
    for (let i = 0; i < 50; i++) {
      ...
      // const categories = item[0].map((category) => category.name);
      const categories = item[0]?.map((category) => category.name) || []; // Use optional chaining and provide a default empty array
