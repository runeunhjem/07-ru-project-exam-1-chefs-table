    ...
    for (const item of result) {
      ...
      const post = {
        ...
        recipe: item.content.rendered
          .match(/Ingredients:([\s\S]*?)<ol> | Instructions:/)[1]
          .replace(/<\/?[^>]+(>|$)/g, "")
          .trim(),
        recipe: item.content.rendered
          .match(/<ol>([\s\S]*?)Instructions:/)[1] // Match everything between the opening <ol> tag and the first occurrence of "Instructions:"
