...
(async function () {
  ...
  postContainer.addEventListener("touchmove", (event) => {
    if (isSwiping) {
      const currentX = event.touches[0].clientX; // get current X position of touch
      diffX = startX - currentX; // calculate X difference between start and end of touch
      postContainer.style.transform = `translateX(-${currentPosition * cardWidth * 3}px)`; // move carousel
    }
  });
  ...
  carousel.addEventListener("touchend", () => {
    if (isSwiping) {
      ...
      postContainer.style.transform = `translateX(-${currentPosition * cardWidth * 3}px)`; // move carousel to final position
      diffX = 0; // reset X difference
    }
  });
