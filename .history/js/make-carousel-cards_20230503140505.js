...
function handleViewportChange(breakpoint) {
  ...
  if (breakpoint >= breakpoints['(max-width: 576px)'] && breakpoint < breakpoints["(max-width: 768px)"]) {
    ...
    postsPerSlide = 2;
  } else if (breakpoint >= breakpoints["(max-width: 1150px)"] && breakpoint < breakpoints["(max-width: 1400px)"]) {
    postsPerSlide = 3;
  } else {
    postsPerSlide = 4;
  };
...
fetchPosts();
In this updated example, we first define a loader element and a carousel element. We also define a new function called fetchPosts() that replaces the existing code. Inside the fetchPosts() function, we add the "active" class to the loader element before calling the getPosts() function. Once the getPosts() function is complete, we process the posts data and add the post elements to the carousel. After that, we add event listeners for the post elements.

You can call the fetchPosts() function when the page loads, or when the user performs a certain action, depending on your specific use case.

You can customize this code by changing the loader element ID and CSS classes to match your existing code.
